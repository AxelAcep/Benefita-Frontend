// components/base/PerusahaanSelect.tsx
    ...
    <div ref={wrapperRef} className="relative">
      ...
      >
        <span className={selected ? "text-zinc-700" : "text-zinc-300"}>
          {selected
            ? `${selected.company ?? "-"} (${selected.noInduk})`
            : placeholder}
        </span>
              ...
              options.map((opt) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 text-xs hover:bg-emerald-50 transition-colors ${
                    selected?.noInduk === opt.noInduk
                      ? "bg-emerald-50 text-emerald-700 font-semibold"
                      : "text-zinc-700"
                  }`}
                >
                  <p className="font-medium">{opt.company ?? "-"}</p>
                  <p className="text-[10px] text-zinc-400">{opt.noInduk}</p>
                </button>
              ))
            )}
          </div>

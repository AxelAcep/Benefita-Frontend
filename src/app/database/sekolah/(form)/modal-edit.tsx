...
import type { InstansiDaerahFormData } from "./card-instansi-daerah";
import type { InformasiInstansiFormData } from "./card-informasi-instansi";
import type { TrainingFormData } from "./card-training";
import type { PrioritasFormData } from "./card-prioritas";
import type { InstansiSekolahFormData } from "./card-instansi-sekolah";
import type { LokasiSekolahFormData } from "./card-lokasi-sekolah";
import type { InformasiSekolahDetailFormData } from "./card-infromasi-sekolah-detail";
...
interface ModalInstansiDaerahProps {
  initialData: InstansiDaerahFormData;
  isSaving: boolean;
  onClose: () => void;
  onSave: (data: InstansiDaerahFormData) => void;
}

export function ModalInstansiDaerah({
  initialData,
  isSaving,
  onClose,
  onSave,
}: ModalInstansiDaerahProps) {
  const [form, setForm] = useState<InstansiDaerahFormData>(initialData);
  ...
  return (
    <ModalWrapper
      title="Edit Instansi Daerah"
      subtitle="Perbarui data instansi daerah."
      onClose={onClose}
      onSave={() => onSave(form)}
      isSaving={isSaving}
    >
...
interface ModalInformasiInstansiProps {
  initialData: InformasiInstansiFormData;
  isSaving: boolean;
  onClose: () => void;
  onSave: (data: InformasiInstansiFormData) => void;
}

export function ModalInformasiInstansi({
  initialData,
  isSaving,
  onClose,
  onSave,
}: ModalInformasiInstansiProps) {
  const [form, setForm] = useState<InformasiInstansiFormData>(initialData);
  ...
  return (
    <ModalWrapper
      title="Edit Informasi Instansi"
      subtitle="Perbarui data informasi instansi."
      onClose={onClose}
      onSave={() => onSave(form)}
      isSaving={isSaving}
    >
...
export function ModalTraining({
  ...
  const [form, setForm] = useState<TrainingFormData>(initialData);
  ...
  return (
    <ModalWrapper
      title="Edit Training"
      subtitle="Perbarui data training."
      onClose={onClose}
      onSave={() => onSave(form)}
      isSaving={isSaving}
    >
...
interface ModalPrioritasProps {
  initialData: PrioritasFormData;
  isSaving: boolean;
  onClose: () => void;
  onSave: (data: PrioritasFormData) => void;
}

export function ModalPrioritas({
  initialData,
  isSaving,
  onClose,
  onSave,
}: ModalPrioritasProps) {
  const [form, setForm] = useState<PrioritasFormData>(initialData);
  ...
  function setField(key: keyof PrioritasFormData, value: string) {
    setForm((prev) => ({ ...prev, [key]: value }) as PrioritasFormData);
  }

  return (
    <ModalWrapper
      title="Edit Prioritas"
      subtitle="Perbarui data prioritas."
      onClose={onClose}
      onSave={() => onSave(form)}
      isSaving={isSaving}
    >
...
interface ModalInstansiSekolahProps {
  initialData: InstansiSekolahFormData;
  isSaving: boolean;
  onClose: () => void;
  onSave: (data: InstansiSekolahFormData) => void;
}

export function ModalInstansiSekolah({
  initialData,
  isSaving,
  onClose,
  onSave,
}: ModalInstansiSekolahProps) {
  const [form, setForm] = useState<InstansiSekolahFormData>(initialData);
  ...
  return (
    <ModalWrapper
      title="Edit Instansi Sekolah"
      subtitle="Perbarui data instansi sekolah."
      onClose={onClose}
      onSave={() => onSave(form)}
      isSaving={isSaving}
    >
...
interface ModalLokasiSekolahProps {
  initialData: LokasiSekolahFormData;
  isSaving: boolean;
  onClose: () => void;
  onSave: (data: LokasiSekolahFormData) => void;
}

export function ModalLokasiSekolah({
  initialData,
  isSaving,
  onClose,
  onSave,
}: ModalLokasiSekolahProps) {
  const [form, setForm] = useState<LokasiSekolahFormData>(initialData);
  ...
  return (
    <ModalWrapper
      ...
      subtitle="Perbarui data lokasi sekolah."
      onClose={onClose}
      onSave={() => onSave(form)}
      isSaving={isSaving}
    >
...
// ─────────────────────────────────────────────
// MODAL INFORMASI SEKOLAH DETAIL
// ─────────────────────────────────────────────

interface ModalInformasiSekolahDetailProps {
  initialData: InformasiSekolahDetailFormData;
  isSaving: boolean;
  onClose: () => void;
  onSave: (data: InformasiSekolahDetailFormData) => void;
}

export function ModalInformasiSekolahDetail({
  initialData,
  isSaving,
  onClose,
  onSave,
}: ModalInformasiSekolahDetailProps) {
  const [form, setForm] = useState<InformasiSekolahDetailFormData>(initialData);
  ...
  return (
    <ModalWrapper
      title="Edit Informasi Sekolah Detail"
      subtitle="Perbarui data informasi sekolah detail."
      onClose={onClose}
      onSave={() => onSave(form)}
      isSaving={isSaving}
    >

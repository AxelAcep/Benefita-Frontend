// app/database/perusahaan/[id]/(tabs)/tab-daily.tsx
"use client";
...
export default function TabDaily({ id }: TabDailyProps) {
  ...
  // null    = modal tertutup
  // "create" = mode tambah
  // string  = mode edit (id daily activity)
  ...
  const handleModalSubmit = (form: DailyActivityFormData) => {
    // pegawaiId → dari user yang login (ambil dari session/context)
    ...
    const payload = {
      ...
      inout: form.inout || "",
      tanggal: isEditMode ? form.tanggal : today,
      perusahaan: id, // otomatis dari props

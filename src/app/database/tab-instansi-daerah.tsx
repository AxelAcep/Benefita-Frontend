...
export default function TabPerusahaan({ jenisInstansi = "INSTANSI_DAERAH" }) {
  const {
    data,
    meta,
    loading,
      ...
      <div className="px-5 py-4 border-b border-zinc-100 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <TableButton
            icon="plus"
            onClick={() => {
              router.push("database/instansi-daerah/create");
            }}
          >
            Tambah Data
          </TableButton>
              ...
              data.map((row, index) => (
                <tr key={row.id} className="border-b border-zinc-50">
                  ...
                  <td className="px-4 py-3 text-xs text-zinc-600">
                    <Anchor
                      name="Lihat Detail"
                      route={`/database/instansi-daerah/${row.id}`}
                    />
                  </td>
                </tr>
              ))

...
export default function KaryawanDetailPage() {
  ...
  const { fetch, loading, data } = useGetRiwayatByPegawai();
  ...
  useEffect(() => {
    if (pegawaiId) fetch(pegawaiId, { page, limit: perPage });
  }, [pegawaiId, page]);

  const riwayat = data?.data ?? [];
  const summary = data?.summary;
  const meta = data?.meta;
  const pegawai = riwayat[0]?.pegawai;

  const totalCutiDanIzin = summary?.totalCutiDanIzin ?? 0;
  const totalSakit = summary?.totalSakit ?? 0;
  const sisaKuota = 12 - totalCutiDanIzin - totalSakit;
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-zinc-800">{totalSakit}</p>
              </div>
              <div className="w-px h-8 bg-zinc-100" />
              <div
                className={`rounded-xl px-4 py-2 ${sisaKuota < 0 ? "bg-red-50" : "bg-zinc-50"}`}
              >
                <p className="text-[11px] text-zinc-400 mb-0.5">
                  Sisa Kuota (Hari)
                </p>
                <p
                  className={`text-xl font-bold ${sisaKuota < 0 ? "text-red-500" : "text-zinc-800"}`}
                >
                  {sisaKuota}
                </p>
              </div>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td
                        colSpan={6}
                        className="px-4 py-8 text-center text-xs text-zinc-300"
                      >
                        Tidak ada data
                      </td>
                    </tr>

...
const DUMMY_DATA: CustomerTraining[] = [
  {
    ...
    kebutuhanTraining: [
      "CSR-01","FI-11","FI-12","PR-08","PR-09","OP-03","LG-05",
      "LG-06","AD-10","EP-19","WM-05","WM-09","HR-01",
      "IT-07","IT-08","MK-01","MK-02","OP-04",
      "SA-15","SA-16","CS-01","CS-02","AD-11","AD-12","RD-01",
    ],
  },
  {
    id: 2,
  ...
  },
  {
    ...
    kebutuhanTraining: [
      "CSR-01","FI-11","FI-12","PR-08","PR-09","OP-03","LG-05",
      "LG-06","AD-10","EP-19","WM-05","WM-09","HR-01",
      "HR-02","IT-05","IT-06","IT-07","MK-01","MK-02","OP-04",
      "SA-15","SA-16","CS-01","AD-11","AD-12","RD-01",
    ],
  },
  {
    id: 4,
  ...
  },
  {
    ...
    kebutuhanTraining: ["CSR-01","HR-01","HR-02","WM-05","AD-10","MK-01","MK-02"],
  },
  {
    id: 6,
    noInduk: "PR00007",
    ...
    kebutuhanTraining: [
      "CSR-01","FI-11","FI-12","EP-19","WM-05","WM-09",
      "IT-07","IT-08","SA-15","SA-16","RD-01",
    ],
  },
];

const KODE_OPTIONS = ["Semua", "CSR-01", "CSR-02", "WM-01", "K3-01", "ENV-01", "HR-05"];
...
const AKUN_COL_STYLE: Record<"env" | "csr" | "tsm" | "epm", React.CSSProperties> = {
...
const AKUN_HEADER_STYLE: Record<"env" | "csr" | "tsm" | "epm", React.CSSProperties> = {
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-zinc-100 space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-zinc-800 text-sm">Kebutuhan Training per Customer</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-semibold text-zinc-500 whitespace-nowrap">Kode Training</span>
            <div className="relative">
              <select
                value={kodeTraining}
            ...
            <button
              onClick={() => { setApplied(kodeTraining); setCurrentPage(1); }}
          ...
          {applied !== "Semua" && (
            <p className="text-[11px] text-zinc-400">
              ...
              <span className="font-semibold text-emerald-600">"{applied}"</span>
            </p>
          )}
        </div>
          ...
          <table className="w-full min-w-[1000px]">
            ...
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50/60">
                ...
                <th className="px-4 py-2 text-[10px] font-semibold text-zinc-400 text-left w-28">No. Induk ↕</th>
                <th className="px-4 py-2 text-[10px] font-semibold text-zinc-400 text-left">Perusahaan/Instansi ↕</th>
                <th className="px-4 py-2 text-[10px] font-semibold text-zinc-400 text-center w-28">Total Peserta</th>
                {/* Akun group header */}
                <th colSpan={4} className="py-2 text-[10px] font-semibold text-zinc-400 text-center">
                  Akun
                </th>
                <th className="px-4 py-2 text-[10px] font-semibold text-zinc-400 text-left">Line of Business</th>
                <th className="px-4 py-2 text-[10px] font-semibold text-zinc-400 text-left w-40">Kebutuhan Training</th>
              </tr>
            ...
            <tbody>
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={10} className="px-4 py-12 text-center text-xs text-zinc-400">
                ...
                paginated.map((row, i) => (
                  <tr key={row.id} className="border-b border-zinc-50 align-top">
                    <td className="px-4 py-3 text-xs text-zinc-400">{(currentPage - 1) * PAGE_SIZE + i + 1}</td>
                    <td className="px-4 py-3 text-xs text-emerald-600 font-semibold cursor-pointer hover:underline whitespace-nowrap">
                      {row.noInduk}
                    </td>
                    <td className="px-4 py-3 text-xs text-zinc-600 whitespace-nowrap">{row.perusahaan}</td>
                    <td className="px-4 py-3 text-xs text-zinc-600 text-center">{row.totalPeserta}</td>
                    {/* Akun 4 kolom — background penuh per kolom, inline style */}
                    <td style={AKUN_COL_STYLE.env} className="px-3 py-3 text-center text-xs font-semibold w-16">{row.akun.env}</td>
                    <td style={AKUN_COL_STYLE.csr} className="px-3 py-3 text-center text-xs font-semibold w-16">{row.akun.csr}</td>
                    <td style={AKUN_COL_STYLE.tsm} className="px-3 py-3 text-center text-xs font-semibold w-16">{row.akun.tsm}</td>
                    <td style={AKUN_COL_STYLE.epm} className="px-3 py-3 text-center text-xs font-semibold w-16">{row.akun.epm}</td>
                    <td className="px-4 py-3 text-xs text-zinc-600 max-w-[200px]">{row.lineOfBusiness}</td>
                    <td className="px-4 py-3 w-[140px] max-w-[140px] overflow-hidden"><TrainingTags tags={row.kebutuhanTraining} /></td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 px-5 py-3 border-t border-zinc-100">
          <p className="text-[11px] text-zinc-400">
            ...
            dari <span className="font-semibold text-zinc-600">{filtered.length}</span> data
          </p>

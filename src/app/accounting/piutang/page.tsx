...
import { usePiutang } from "@/hooks/use-piutang";
import { PiutangItem } from "@/lib/services/accounting.service";
import ModalDetailPiutang from "./ModalDetail";
...
export default function PiutangPage() {
  ...
  const [sortBy, setSortBy] = useState<
    "totalPendapatan" | "totalPendapatan" | "totalPiutang" | "tglSelesai"
  >("totalPiutang");
  ...
  const getApiParams = useCallback(() => {
    const params: any = {
      page: 1, // selalu 1 karena kita ambil semua data
      limit,
      sortBy,
      order: sortOrder,
    };
    if (activeTab !== "ALL") params.jenis = activeTab;
    if (isCustomRange) {
  ...
  const { data, grandTotal, loading, error, refetch } =
    usePiutang(getApiParams());

  // ─── Effect: refresh saat filter berubah (kecuali page & search) ──
  useEffect(() => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(
      (item) =>
        item.kodePelatihan.toLowerCase().includes(s) ||
        item.judulLengkap.toLowerCase().includes(s) ||
        item.judulTraining.toLowerCase().includes(s),
    );
  }, [data, search]);
  ...
  // ─── Handlers ──────────────────────────────────────────────────
  const handleTabChange = (tab: JenisTab) => setActiveTab(tab);
  const handleApplyCustomRange = () => setIsCustomRange(true);
  const handleUseCurrentMonth = () => setIsCustomRange(false);

  const handleOpenDetail = (kodeJadwal: string) => {
    setSelectedJadwal(kodeJadwal);
    setIsModalOpen(true);
  };
        ...
        <div className="px-6 py-4 bg-white border-b border-zinc-100 flex items-center justify-between">
          <div>
            <p className="text-xs text-zinc-400">
              Perusahaan &rsaquo;{" "}
              <span className="font-semibold text-zinc-700">Rekap Piutang</span>
            </p>
          ...
          <div className="flex flex-wrap items-center gap-4 bg-white p-3 rounded-xl border border-zinc-200 shadow-sm">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-1.5 rounded-lg border border-zinc-200 hover:bg-zinc-50 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="border border-zinc-200 rounded-lg px-2 py-1 text-xs"
              >
                <option value="totalPesertaBelumLunas">
                  Total Belum Bayar
                </option>
                <option value="totalPendapatan">Total Pendapatan</option>
                <option value="totalPiutang">Total Piutang</option>
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-zinc-100">
              <div className="flex items-center gap-3">
                ...
                <p className="font-bold text-zinc-800 text-sm">Rekap Piutang</p>
              </div>
              {/* Search */}
                ...
                <tbody>
                  {paginatedData.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-zinc-700 text-center">
                        {row.totalPesertaFix - row.totalPesertaBelumLunas} /{" "}
                        {row.totalPesertaFix}
                      </td>
                      <td className="px-4 py-3 text-xs text-red-400 text-center">
                        {row.totalPesertaBelumLunas}
                      </td>
                      <td className="px-4 py-3 text-xs text-zinc-700">
                        {formatRupiah(row.totalPendapatan - row.totalPiutang)} /{" "}
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => handleOpenDetail(row.kodeJadwal)}
            ...
            <div className="flex flex-wrap items-center justify-end gap-6 px-5 py-3 bg-zinc-50 border-t border-zinc-100">
              ...
              <div className="flex items-center gap-1 text-xs">
                <span className="text-zinc-500">Total Belum Bayar:</span>
                <span className="font-bold text-red-600">
                  {grandTotal.totalBelumBayar}
                </span>
              </div>
              <div className="flex items-center gap-1 text-xs">
                <span className="text-zinc-500">Total Piutang:</span>
                <span className="font-bold text-red-600">
                  {formatRupiah(grandTotal.totalPiutang)}
                </span>
              </div>
            </div>
          </div>
